LearnsetPointers::
	table_width 2, LearnsetPointers
	dw BulbasaurLearnset
	dw IvysaurLearnset
	dw VenusaurLearnset
	dw CharmanderLearnset
	dw CharmeleonLearnset
	dw CharizardLearnset
	dw SquirtleLearnset
	dw WartortleLearnset
	dw BlastoiseLearnset
	dw CaterpieLearnset
	dw MetapodLearnset
	dw ButterfreeLearnset
	dw WeedleLearnset
	dw KakunaLearnset
	dw BeedrillLearnset
	dw PidgeyLearnset
	dw PidgeottoLearnset
	dw PidgeotLearnset
	dw RattataPlainLearnset
	dw RaticatePlainLearnset
	dw SpearowLearnset
	dw FearowLearnset
	dw EkansLearnset
	dw ArbokLearnset
	dw PikachuLearnset
	dw RaichuPlainLearnset
	dw SandshrewPlainLearnset
	dw SandslashPlainLearnset
	dw NidoranFLearnset
	dw NidorinaLearnset
	dw NidoqueenLearnset
	dw NidoranMLearnset
	dw NidorinoLearnset
	dw NidokingLearnset
	dw ClefairyLearnset
	dw ClefableLearnset
	dw VulpixPlainLearnset
	dw NinetalesPlainLearnset
	dw JigglypuffLearnset
	dw WigglytuffLearnset
	dw ZubatLearnset
	dw GolbatLearnset
	dw OddishLearnset
	dw GloomLearnset
	dw VileplumeLearnset
	dw ParasLearnset
	dw ParasectLearnset
	dw VenonatLearnset
	dw VenomothLearnset
	dw DiglettPlainLearnset
	dw DugtrioPlainLearnset
	dw MeowthPlainLearnset
	dw PersianPlainLearnset
	dw PsyduckLearnset
	dw GolduckLearnset
	dw MankeyLearnset
	dw PrimeapeLearnset
	dw GrowlithePlainLearnset
	dw ArcaninePlainLearnset
	dw PoliwagLearnset
	dw PoliwhirlLearnset
	dw PoliwrathLearnset
	dw AbraLearnset
	dw KadabraLearnset
	dw AlakazamLearnset
	dw MachopLearnset
	dw MachokeLearnset
	dw MachampLearnset
	dw BellsproutLearnset
	dw WeepinbellLearnset
	dw VictreebelLearnset
	dw TentacoolLearnset
	dw TentacruelLearnset
	dw GeodudePlainLearnset
	dw GravelerPlainLearnset
	dw GolemPlainLearnset
	dw PonytaPlainLearnset
	dw RapidashPlainLearnset
	dw SlowpokePlainLearnset
	dw SlowbroPlainLearnset
	dw MagnemiteLearnset
	dw MagnetonLearnset
	dw FarfetchDPlainLearnset
	dw DoduoLearnset
	dw DodrioLearnset
	dw SeelLearnset
	dw DewgongLearnset
	dw GrimerPlainLearnset
	dw MukPlainLearnset
	dw ShellderLearnset
	dw CloysterLearnset
	dw GastlyLearnset
	dw HaunterLearnset
	dw GengarLearnset
	dw OnixLearnset
	dw DrowzeeLearnset
	dw HypnoLearnset
	dw KrabbyLearnset
	dw KinglerLearnset
	dw VoltorbLearnset
	dw ElectrodeLearnset
	dw ExeggcuteLearnset
	dw ExeggutorPlainLearnset
	dw CuboneLearnset
	dw MarowakPlainLearnset
	dw HitmonleeLearnset
	dw HitmonchanLearnset
	dw KoffingLearnset
	dw WeezingPlainLearnset
	dw RhyhornLearnset
	dw RhydonLearnset
	dw ChanseyLearnset
	dw TangelaLearnset
	dw KangaskhanLearnset
	dw HorseaLearnset
	dw SeadraLearnset
	dw TogepiLearnset
	dw TogeticLearnset
	dw TogekissLearnset
	dw StaryuLearnset
	dw StarmieLearnset
	dw MrMimePlainLearnset
	dw ScytherLearnset
	dw JynxLearnset
	dw ElectabuzzLearnset
	dw MagmarLearnset
	dw PinsirLearnset
	dw TaurosLearnset
	dw MagikarpLearnset
	dw GyaradosLearnset
	dw LaprasLearnset
	dw DittoLearnset
	dw EeveeLearnset
	dw VaporeonLearnset
	dw JolteonLearnset
	dw FlareonLearnset
	dw PorygonLearnset
	dw OmanyteLearnset
	dw OmastarLearnset
	dw KabutoLearnset
	dw KabutopsLearnset
	dw AerodactylLearnset
	dw SnorlaxLearnset
	dw ArticunoPlainLearnset
	dw ZapdosPlainLearnset
	dw MoltresPlainLearnset
	dw DratiniLearnset
	dw DragonairLearnset
	dw DragoniteLearnset
	dw MewtwoLearnset
	dw MewLearnset
	dw ChikoritaLearnset
	dw BayleefLearnset
	dw MeganiumLearnset
	dw CyndaquilLearnset
	dw QuilavaLearnset
	dw TyphlosionLearnset
	dw TotodileLearnset
	dw CroconawLearnset
	dw FeraligatrLearnset
	dw SentretLearnset
	dw FurretLearnset
	dw HoothootLearnset
	dw NoctowlLearnset
	dw GoldeenLearnset
	dw SeakingLearnset
	dw SpinarakLearnset
	dw AriadosLearnset
	dw CrobatLearnset
	dw ChinchouLearnset
	dw LanturnLearnset
	dw PichuLearnset
	dw MunchlaxLearnset
	dw MagnezoneLearnset
	dw RhyperiorLearnset
	dw TangrowthLearnset
	dw NatuLearnset
	dw XatuLearnset
	dw MareepLearnset
	dw FlaaffyLearnset
	dw AmpharosLearnset
	dw BellossomLearnset
	dw LickilickyLearnset
	dw PolitoedLearnset
	dw SunkernLearnset
	dw SunfloraLearnset
	dw YanmaLearnset
	dw YanmegaLearnset
	dw WooperPlainLearnset
	dw QuagsireLearnset
	dw EspeonLearnset
	dw UmbreonLearnset
	dw MurkrowLearnset
	dw HonchkrowLearnset
	dw SlowkingPlainLearnset
	dw MisdreavusLearnset
	dw MismagiusLearnset
	dw UnownLearnset
	dw SirfetchDLearnset
	dw MrRimeLearnset
	dw PinecoLearnset
	dw ForretressLearnset
	dw PerrserkerLearnset
	dw GligarLearnset
	dw GliscorLearnset
	dw SteelixLearnset
	dw SnubbullLearnset
	dw GranbullLearnset
	dw QwilfishLearnset
	dw ScizorLearnset
	dw ShuckleLearnset
	dw HeracrossLearnset
	dw SneaselPlainLearnset
	dw WeavileLearnset
	dw TeddiursaLearnset
	dw UrsaringLearnset
	dw SlugmaLearnset
	dw MagcargoLearnset
	dw SwinubLearnset
	dw PiloswineLearnset
	dw MamoswineLearnset
	dw HappinyLearnset
	dw SmoochumLearnset
	dw MimeJrLearnset
	dw LickitungLearnset
	dw IgglybuffLearnset
	dw HoundourLearnset
	dw HoundoomLearnset
	dw KingdraLearnset
	dw PhanpyLearnset
	dw DonphanLearnset
	dw Porygon2Learnset
	dw PorygonZLearnset
	dw CleffaLearnset
	dw TyrogueLearnset
	dw HitmontopLearnset
	dw ElekidLearnset
	dw ElectivireLearnset
	dw MagbyLearnset
	dw MagmortarLearnset
	dw MiltankLearnset
	dw BlisseyLearnset
	dw RaikouLearnset
	dw EnteiLearnset
	dw SuicuneLearnset
	dw LarvitarLearnset
	dw PupitarLearnset
	dw TyranitarLearnset
	dw LugiaLearnset
	dw HoOhLearnset
	dw CelebiLearnset
	dw LeafeonLearnset
	dw GlaceonLearnset
	dw SylveonLearnset
	assert_table_length NUM_POKEMON

	dw NoLearnset ; Egg

	dw GyaradosLearnset
	dw MewtwoLearnset
	dw RattataAlolanLearnset
	dw RaticateAlolanLearnset
	dw SandshrewAlolanLearnset
	dw SandslashAlolanLearnset
	dw VulpixAlolanLearnset
	dw NinetalesAlolanLearnset
	dw DiglettAlolanLearnset
	dw DugtrioAlolanLearnset
	dw MeowthAlolanLearnset
	dw PersianAlolanLearnset
	dw GeodudeAlolanLearnset
	dw GravelerAlolanLearnset
	dw GolemAlolanLearnset
	dw GrimerAlolanLearnset
	dw MukAlolanLearnset
	dw RaichuAlolanLearnset
	dw ExeggutorAlolanLearnset
	dw MarowakAlolanLearnset
	dw MeowthGalarianLearnset
	dw PonytaGalarianLearnset
	dw RapidashGalarianLearnset
	dw SlowpokeGalarianLearnset
	dw SlowbroGalarianLearnset
	dw SlowkingGalarianLearnset
	dw FarfetchDGalarianLearnset
	dw WeezingGalarianLearnset
	dw MrMimeGalarianLearnset
	dw ArticunoGalarianLearnset
	dw ZapdosGalarianLearnset
	dw MoltresGalarianLearnset
	assert_table_length NUM_EXT_POKEMON

Learnsets::

BulbasaurLearnset:
IvysaurLearnset:
VenusaurLearnset:
	db 1, STRIKE ; TACKLE
	db 3, GROWL
	db 7, VINE_WHIP
	db 9, LEECH_SEED
	db 14, POISONPOWDER
	db 14, SLEEP_POWDER
	db 22, TAKE_DOWN
	db 30, RAZOR_LEAF
	db 42, GROWTH
	db 51, HEALINGLIGHT
	db 60, BULLET_SEED
	db 69, DOUBLE_EDGE
	db 78, SOLAR_BEAM
	db 0 ; no more level-up moves

CharmanderLearnset:
CharmeleonLearnset:
CharizardLearnset:
	db 1, STRIKE ; SCRATCH
	db 1, GROWL
	db 5, EMBER
	db 9, SMOKESCREEN
	db 14, FURY_STRIKES
	db 24, DRAGONBREATH
	db 32, FIRE_SPIN
	db 43, SLASH
	db 54, FLAMETHROWER
	db 62, AIR_SLASH
	db 71, FLARE_BLITZ
	db 0 ; no more level-up moves

SquirtleLearnset:
WartortleLearnset:
BlastoiseLearnset:
	db 1, STRIKE ; TACKLE
	db 4, LEER ; Tail Whip → similar move
	db 7, WATER_GUN
	db 10, HARDEN ; DEFENSE_CURL/WITHDRAW
	db 14, RAPID_SPIN
	db 20, BUBBLE_BEAM
	db 26, BITE
	db 33, RAIN_DANCE
	db 45, AQUA_TAIL
	db 50, PROTECT
	db 60, HEADBUTT
	db 70, HYDRO_PUMP
	db 75, CLOSE_COMBAT ; Skull_Bash
	db 0 ; no more level-up moves

CaterpieLearnset:
	db 1, STRIKE ; TACKLE
	db 1, STRING_SHOT
	db 9, BUG_BITE
	db 0 ; no more level-up moves

MetapodLearnset:
KakunaLearnset:
	db 1, HARDEN
	db 0 ; no more level-up moves

ButterfreeLearnset:
	db 1, STRIKE ; TACKLE ; Caterpie move
	db 1, STRING_SHOT ; Caterpie move
	db 1, GUST ; Evo move
	db 11, CONFUSION
	db 13, POISONPOWDER
	db 13, STUN_SPORE
	db 13, SLEEP_POWDER ; Psybeam → Sleep Powder
	db 16, PSYBEAM ; Silver Wind → Psybeam
	db 19, SUPERSONIC
	db 22, QUIVER_DANCE
	db 25, HYPNOSIS ; Whirlwind → new move
	db 28, AIR_SLASH
	db 31, BUG_BUZZ
	db 0 ; no more level-up moves

WeedleLearnset:
	db 1, POISON_STING
	db 1, STRING_SHOT
	db 9, BUG_BITE
	db 0 ; no more level-up moves

BeedrillLearnset:
	db 1, POISON_STING ; Weedle move
	db 1, STRING_SHOT ; Weedle move
	db 1, FURY_STRIKES ; Fury Attack → similar move
	db 12, FURY_CUTTER
	db 16, PURSUIT
	db 20, POISON_JAB
	db 23, VENOSHOCK ; Add Move
	db 24, AGILITY
	db 28, PIN_MISSILE
	db 32, OUTRAGE ; Fell Stinger → Let's Go move
	db 0 ; no more level-up moves

PidgeyLearnset:
PidgeottoLearnset:
PidgeotLearnset:
	db 1, STRIKE ; TACKLE
	db 3, SAND_ATTACK
	db 5, GUST
	db 9, QUICK_ATTACK
	db 12, WING_ATTACK
	db 16, ROOST
	db 23, ROAR ; WHIRLWIND
	db 31, AGILITY
	db 43, AIR_SLASH
	db 53, DRILL_PECK ; RAZOR_WIND → new move
	db 63, HURRICANE
	db 0 ; no more level-up moves

RattataPlainLearnset:
RattataAlolanLearnset:
RaticatePlainLearnset:
RaticateAlolanLearnset:
	db 1, STRIKE ; TACKLE
	db 1, LEER ; Tail Whip → similar move
	db 6, QUICK_ATTACK
	db 9, BITE
	db 12, TAKE_DOWN
	db 15, CRUNCH
	db 18, PURSUIT
	db 24, SUPER_FANG
	db 30, SUCKER_PUNCH
	db 36, HYPER_FANG
	db 42, DOUBLE_EDGE
	db 0 ; no more level-up moves

SpearowLearnset:
FearowLearnset:
	db 1, DRILL_RUN
	db 1, PECK
	db 1, GROWL
	db 8, LEER
	db 11, PURSUIT
	db 16, FURY_STRIKES ; Fury Attack → similar move
	db 19, WING_ATTACK
	db 29, ROOST
	db 37, AGILITY
	db 47, DRILL_PECK
	db 0 ; no more level-up moves

EkansLearnset:
ArbokLearnset:
	db 1, WRAP
	db 1, POISON_STING
	db 1, CRUNCH ; evolution move
	db 4, LEER
	db 8, ACID
	db 12, BITE
	db 16, HAZE
	db 20, GLARE
	db 24, POISON_JAB
	db 28, BODY_SLAM
	db 32, SCREECH
	db 36, TOXIC
	db 40, BULK_UP ; Coil → TM move
	db 0 ; no more level-up moves

PichuLearnset:
PikachuLearnset:
RaichuPlainLearnset:
RaichuAlolanLearnset:
	db 1, THUNDERSHOCK
	db 1, GROWL
	db 3, LEER ; Tail Whip → similar move
	db 6, QUICK_ATTACK
	db 9, DOUBLE_KICK
	db 12, DOUBLE_TEAM
	db 15, THUNDER_WAVE
	db 18, LIGHT_SCREEN
	db 21, THUNDERBOLT
	db 24, BODY_SLAM
	db 27, AGILITY
	db 30, THUNDER
	db 0 ; no more level-up moves

SandshrewPlainLearnset:
SandslashPlainLearnset:
	db 1, STRIKE ; SCRATCH
	db 1, DEFENSE_CURL
	db 3, SAND_ATTACK
	db 6, POISON_STING
	db 9, ROLLOUT
	db 12, FURY_CUTTER
	db 15, RAPID_SPIN
	db 18, BULLDOZE
	db 21, SWIFT
	db 24, FURY_STRIKES ; Fury Swipes → similar move
	db 27, AGILITY
	db 30, SLASH
	db 33, DIG
	db 36, SWORDS_DANCE
	db 39, SANDSTORM
	db 42, EARTHQUAKE
	db 0 ; no more level-up moves

SandshrewAlolanLearnset:
SandslashAlolanLearnset:
	db 1, STRIKE ; SCRATCH
	db 1, DEFENSE_CURL
	db 3, HAZE
	db 6, ICE_SHARD
	db 9, ROLLOUT
	db 9, RAPID_SPIN
	db 12, FURY_CUTTER
	db 15, RAPID_SPIN
	db 18, METAL_CLAW
	db 21, SWIFT
	db 24, FURY_STRIKES ; Fury Swipes → similar move
	db 27, MIRROR_COAT
	db 30, SLASH
	db 33, IRON_HEAD
	db 36, SWORDS_DANCE
	db 39, HAIL
	db 42, BLIZZARD
	db 0 ; no more level-up moves

NidoranFLearnset:
NidorinaLearnset:
NidoqueenLearnset:
NidoranMLearnset:
NidorinoLearnset:
NidokingLearnset:
	db 1, MEGAHORN
	db 1, PECK
	db 1, DOUBLE_KICK
	db 1, POISON_STING
	db 25, OUTRAGE ; Chip Away → HGSS tutor move
	db 35, THRASH
	db 45, MEGAHORN
	db 0 ; no more level-up moves

CleffaLearnset:
ClefairyLearnset:
ClefableLearnset:
	db 1, HEALINGLIGHT ; Moonlight → similar move
	db 1, PLAY_ROUGH ; Let's Go TM move
	db 1, SING
	db 1, MINIMIZE
	db 1, METRONOME
	db 0 ; no more level-up moves

VulpixPlainLearnset:
NinetalesPlainLearnset:
	db 1, EMBER
	db 3, LEER ; Tail Whip → new move
	db 6, EMBER
	db 9, QUICK_ATTACK
	db 12, CONFUSE_RAY
	db 15, WILL_O_WISP
	db 18, SUNNY_DAY
	db 21, FIRE_SPIN
	db 24, FLAMETHROWER
	db 27, ROAR
	db 30, BARRIER
	db 36, FIRE_BLAST
	db 0 ; no more level-up moves

VulpixAlolanLearnset:
NinetalesAlolanLearnset:
	db 1, EMBER
	db 3, LEER ; Tail Whip → new move
	db 6, ICE_SHARD
	db 9, HAZE
	db 12, CONFUSE_RAY
	db 15, AURORA_BEAM
	db 18, HAIL
	db 21, DAZZLINGLEAM
	db 24, ICE_BEAM
	db 27, ROAR
	db 30, AURORA_VEIL
	db 36, BLIZZARD
	db 0 ; no more level-up moves

IgglybuffLearnset:
JigglypuffLearnset:
WigglytuffLearnset:
	db 1, MINIMIZE ; Let's Go move
	db 1, DOUBLE_EDGE
	db 1, PLAY_ROUGH
	db 1, SING
	db 1, DEFENSE_CURL
	db 1, DISABLE
	db 0 ; no more level-up moves

ZubatLearnset:
GolbatLearnset:
CrobatLearnset:
	db 1, ABSORB
	db 1, SUPERSONIC
	db 4, WING_ATTACK
	db 8, BITE
	db 12, SWIFT
	db 16, SUPER_FANG ; POISON_FANG
	db 20, CONFUSE_RAY
	db 24, HAZE
	db 28, LEECH_LIFE
	db 32, TOXIC
	db 36, AIR_SLASH
	db 0 ; no more level-up moves

OddishLearnset:
GloomLearnset:
VileplumeLearnset:
BellossomLearnset:
	db 1, MEGA_DRAIN
	db 1, POISONPOWDER
	db 1, STUN_SPORE
	db 39, GROWTH
	db 49, MOONBLAST ; Petal Blizzard → Gloom move
	db 59, PETAL_DANCE
	db 69, SOLAR_BEAM
	db 0 ; no more level-up moves

ParasLearnset:
ParasectLearnset:
	db 1, STRIKE ; SCRATCH
	db 3, SLEEP_POWDER
	db 6, STUN_SPORE
	db 9, POISONPOWDER
	db 12, ABSORB
	db 15, GROWTH
	db 18, FURY_STRIKES
	db 21, FURY_CUTTER
	db 24, SLASH
	db 27, LEECH_LIFE
	db 30, SPORE
	db 33, X_SCISSOR
	db 36, GRASS_KNOT ; ENERGY_BALL
	db 0 ; no more level-up moves

VenonatLearnset:
VenomothLearnset:
	db 1, STRIKE ; TACKLE
	db 4, DISABLE
	db 8, SUPERSONIC
	db 12, CONFUSION
	db 14, POISONPOWDER
	db 16, STUN_SPORE
	db 20, PSYBEAM
	db 24, SLEEP_POWDER
	db 28, LEECH_LIFE
	db 32, PSYCHIC_M
	db 59, BUG_BUZZ ;POISON_FANG
	db 0 ; no more level-up moves

DiglettPlainLearnset:
DugtrioPlainLearnset:
	db 1, STRIKE ; SCRATCH
	db 1, SAND_ATTACK
	db 4, GROWL
	db 8, AGILITY
	db 12, FURY_STRIKES
	db 16, DIG
	db 20, BULLDOZE
	db 24, SUCKER_PUNCH
	db 28, SLASH
	db 32, SANDSTORM
	db 36, EARTHQUAKE
	db 0 ; no more level-up moves

DiglettAlolanLearnset:
DugtrioAlolanLearnset:
	db 1, METAL_CLAW
	db 1, SAND_ATTACK
	db 4, GROWL
	db 8, AGILITY
	db 12, FURY_STRIKES
	db 16, DIG
	db 20, BULLDOZE
	db 24, SUCKER_PUNCH
	db 28, IRON_HEAD
	db 32, SANDSTORM
	db 36, EARTHQUAKE
	db 0 ; no more level-up moves

MeowthPlainLearnset:
PersianPlainLearnset:
	db 1, STRIKE ; SCRATCH
	db 1, GROWL
	db 3, BITE
	db 6, FAKE_OUT
	db 9, TAUNT
	db 12, PAY_DAY
	db 15, FEINT
	db 19, FURY_STRIKES ; Fury Swipes → similar move
	db 23, SCREECH
	db 27, SLASH
	db 31, NASTY_PLOT
	db 35, PLAY_ROUGH
	db 0 ; no more level-up moves

MeowthAlolanLearnset:
PersianAlolanLearnset:
	db 1, STRIKE ; SCRATCH
	db 1, GROWL
	db 3, BITE
	db 6, FAKE_OUT
	db 9, TAUNT
	db 12, PAY_DAY
	db 15, FEINT
	db 19, FURY_STRIKES ; Fury Swipes → similar move
	db 23, SCREECH
	db 27, NIGHT_SLASH
	db 31, NASTY_PLOT
	db 35, PLAY_ROUGH
	db 0 ; no more level-up moves

MeowthGalarianLearnset:
PerrserkerLearnset:
	db 1, STRIKE ; SCRATCH
	db 1, GROWL
	db 3, METAL_CLAW
	db 6, FAKE_OUT
	db 9, TAUNT
	db 12, PAY_DAY
	db 15, FEINT
	db 19, FURY_STRIKES ; Fury Swipes → similar move
	db 23, SCREECH
	db 27, SLASH
	db 31, NASTY_PLOT
	db 35, PLAY_ROUGH
	db 0 ; no more level-up moves

PsyduckLearnset:
GolduckLearnset:
	db 1, STRIKE ; SCRATCH
	db 4, LEER ; Tail Whip → similar move
	db 8, WATER_GUN
	db 12, CONFUSION
	db 16, FURY_STRIKES ; Fury Swipes → similar move
	db 20, DISABLE
	db 24, WATER_PULSE
	db 28, PSYBEAM
	db 32, AQUA_TAIL
	db 36, SCREECH
	db 40, SURF
	db 44, AMNESIA
	db 48, HYDRO_PUMP
	db 0 ; no more level-up moves

MankeyLearnset:
PrimeapeLearnset:
AnnihilapeLearnset:
	db 1, STRIKE ; SCRATCH
	db 1, LEER
	db 5, FURY_STRIKES ; Fury Swipes → similar move
	db 10, LOW_KICK
	db 15, TAUNT
	db 20, SEISMIC_TOSS
	db 25, CROSS_CHOP
	db 30, SCREECH
	db 35, RAGE_FIST ; New move. Triggers evolution
	db 40, THRASH
	db 45, CLOSE_COMBAT
	db 50, OUTRAGE
	db 0 ; no more level-up moves

GrowlithePlainLearnset:
ArcaninePlainLearnset:
	db 1, BITE
	db 1, ROAR
	db 3, EMBER
	db 7, LEER
	db 12, TAKE_DOWN
	db 17, FLAMETHROWER
	db 21, AGILITY
	db 26, CRUNCH
	db 31, OUTRAGE
	db 35, PLAY_ROUGH
	db 38, FLARE_BLITZ
	db 0 ; no more level-up moves

GrowlitheHisuianLearnset:
ArcanineHisuianLearnset:
	db 1, BITE
	db 1, ROAR
	db 3, EMBER
	db 7, LEER
	db 12, TAKE_DOWN
	db 17, FLAMETHROWER
	db 21, AGILITY
	db 26, CRUNCH
	db 31, OUTRAGE
	db 35, ROCK_SLIDE
	db 38, FLARE_BLITZ
	db 0 ; no more level-up moves

PoliwagLearnset:
PoliwhirlLearnset:
PoliwrathLearnset:
PolitoedLearnset:
	db 1, WATER_GUN
	db 4, STRIKE ; POUND
	db 8, HYPNOSIS
	db 12, FURY_STRIKES ; Fury Swipes → similar move
	db 16, BUBBLE_BEAM
	db 20, RAIN_DANCE
	db 24, LOW_KICK
	db 28, BODY_SLAM
	db 32, HYDRO_PUMP
	db 36, BELLY_DRUM
	db 40, DOUBLE_EDGE
	db 0 ; no more level-up moves

AbraLearnset:
	db 1, TELEPORT
	db 0 ; no more level-up moves

KadabraLearnset:
AlakazamLearnset:
	db 1, CALM_MIND
	db 1, BARRIER
	db 1, PSYBEAM ; evolution move
	db 1, CONFUSION
	db 1, TELEPORT
	db 6, DISABLE
	db 12, NIGHT_SHADE ; Miracle Eye → Let's Go move
	db 18, SUBSTITUTE
	db 24, PSYCHO_CUT
	db 30, REFLECT
	db 36, RECOVER
	db 42, PSYCHIC_M
	db 0 ; no more level-up moves

MachopLearnset:
MachokeLearnset:
MachampLearnset:
	db 1, LOW_KICK
	db 1, LEER
	db 6, SEISMIC_TOSS
	db 12, BRICK_BREAK
	db 18, LOW_SWEEP
	db 24, BULK_UP
	db 30, DYNAMICPUNCH
	db 36, CROSS_CHOP
	db 42, SUPERPOWER
	db 48, CLOSE_COMBAT
	db 54, DOUBLE_EDGE
	db 0 ; no more level-up moves

BellsproutLearnset:
WeepinbellLearnset:
VictreebelLearnset:
	db 1, VINE_WHIP
	db 1, SLEEP_POWDER
	db 1, HEALINGLIGHT
	db 1, RAZOR_LEAF
	db 1, LEAF_BLADE
	db 32, SUCKER_PUNCH
	db 44, SWORDS_DANCE
	db 0 ; no more level-up moves

TentacoolLearnset:
TentacruelLearnset:
	db 1, POWER_WHIP ; evolution move
	db 1, POISON_STING
	db 4, SUPERSONIC
	db 7, WATER_GUN ; Constrict → new move
	db 10, ACID
	db 16, WATER_PULSE
	db 19, WRAP
	db 22, CONFUSE_RAY ; Acid Spray → event move
	db 25, BUBBLE_BEAM
	db 28, BARRIER
	db 32, POISON_JAB
	db 36, VENOSHOCK ; Brine → tutor move
	db 40, SCREECH
	db 44, HEX
	db 48, SLUDGE_BOMB ; Sludge Wave → TM move
	db 52, HYDRO_PUMP
	db 0 ; no more level-up moves

GeodudePlainLearnset:
GeodudeAlolanLearnset:
GravelerPlainLearnset:
GravelerAlolanLearnset:
GolemPlainLearnset:
GolemAlolanLearnset:
	db 1, STRIKE ; TACKLE
	db 1, DEFENSE_CURL
	db 6, RAPID_SPIN ; Rock Polish → event move
	db 10, ROLLOUT
	db 16, ROCK_THROW
	db 18, ANCIENTPOWER ; Smack Down → HGSS tutor move
	db 22, BULLDOZE
	db 24, ROCK_BLAST ; Self-Destruct → Rock Blast
	db 30, SANDSTORM ; Stealth Rock → TM move
	db 34, ROCK_SLIDE ; Rock Blast → TM move
	db 40, EARTHQUAKE
	db 44, EXPLOSION
	db 50, DOUBLE_EDGE
	db 54, STONE_EDGE
	db 60, GYRO_BALL ; Heavy Slam → similar move
	db 0 ; no more level-up moves

PonytaPlainLearnset:
PonytaGalarianLearnset:
RapidashPlainLearnset:
RapidashGalarianLearnset:
	db 1, PLAY_ROUGH ; evolution move
	db 1, MEGAHORN
	db 1, GROWL
	db 1, STRIKE ; TACKLE
	db 4, LEER ; Tail Whip → similar move
	db 9, CONFUSION
	db 13, DISARM_VOICE ; Fairy Wind → similar move
	db 17, STOMP
	db 21, HEALINGLIGHT ; Heal Pulse → similar move
	db 25, PSYBEAM
	db 29, TAKE_DOWN
	db 33, DAZZLINGLEAM
	db 37, AGILITY
	db 41, PSYCHIC_M
	db 45, EXTREMESPEED ; Bounce → new move
	db 49, MOONBLAST ; Healing Wish → new move
	db 0 ; no more level-up moves

SlowpokePlainLearnset:
SlowpokeGalarianLearnset:
SlowbroPlainLearnset:
SlowbroGalarianLearnset:
SlowkingPlainLearnset:
SlowkingGalarianLearnset:
	db 1, POISON_JAB ; Shell Side Arm → similar move ; evolution move
	db 1, STRIKE ; TACKLE
	db 5, GROWL
	db 9, ACID
	db 14, CONFUSION
	db 19, DISABLE
	db 23, HEADBUTT
	db 28, WATER_PULSE
	db 32, ZEN_HEADBUTT
	db 43, AMNESIA
	db 49, PSYCHIC_M
	db 55, RAIN_DANCE
	db 62, BELLY_DRUM ; Psych Up → egg move
	db 68, RECOVER ; Heal Pulse → similar move
	db 0 ; no more level-up moves

MagnemiteLearnset:
MagnetonLearnset:
MagnezoneLearnset:
	db 1, TRI_ATTACK ; evolution move
	db 1, SUPERSONIC
	db 1, STRIKE ; TACKLE
	db 5, THUNDERSHOCK
	db 7, THUNDER_WAVE
	db 13, LIGHT_SCREEN
	db 23, SWIFT ; Mirror Shot → TM move
	db 25, SCREECH ; Metal Sound → Screech
	db 29, THUNDERBOLT ; Electro Ball → TM move
	db 33, FLASH_CANNON
	db 39, EXPLOSION ; Screech → TM move
	db 49, AGILITY ; Lock-On → event move
	db 53, GYRO_BALL ; Magnet Rise → Gyro Ball
	db 59, THUNDER ; Gyro Ball → TM move
	db 0 ; no more level-up moves

FarfetchDPlainLearnset:
FarfetchDGalarianLearnset:
SirfetchDLearnset:
	db 1, POISON_JAB
	db 1, PECK
	db 1, SAND_ATTACK
	db 1, LEER
	db 7, FURY_STRIKES ; Fury Attack → similar move
	db 9, AERIAL_ACE
	db 13, KNOCK_OFF
	db 15, RAZOR_LEAF ; Let's Go move
	db 19, SLASH
	db 21, LOW_SWEEP ; Air Cutter → new move
	db 25, SWORDS_DANCE
	db 31, AGILITY
	db 33, NIGHT_SLASH
	db 37, ACROBATICS
	db 43, HI_JUMP_KICK ; Feint → new move
	db 45, FURY_CUTTER
	db 49, AIR_SLASH
	db 55, BRAVE_BIRD
	db 0 ; no more level-up moves

DoduoLearnset:
DodrioLearnset:
	db 1, TRI_ATTACK ; evolution move
	db 1, PECK
	db 1, GROWL
	db 5, QUICK_ATTACK
	db 9, FURY_CUTTER
	db 12, FURY_STRIKES ; Fury Attack → similar move
	db 15, PURSUIT
	db 22, DOUBLE_KICK ; Double Hit → new move
	db 26, AGILITY
	db 29, DOUBLE_TEAM ; Uproar → TM move
	db 34, BULLDOZE ; Acupressure → TM move
	db 38, SWORDS_DANCE
	db 43, HI_JUMP_KICK ; Jump Kick → similar move
	db 47, DRILL_PECK
	db 52, FLAIL ; Endeavor → new move
	db 56, THRASH
	db 0 ; no more level-up moves

SeelLearnset:
DewgongLearnset:
	db 1, ICICLE_SPEAR ; Sheer Cold → egg move ; evolution move
	db 1, HEADBUTT
	db 3, GROWL
	db 7, WATER_GUN ; Signal Beam → RBY TM move
	db 11, ICY_WIND
	db 13, ENCORE
	db 17, ICE_SHARD
	db 21, REST
	db 27, AURORA_BEAM
	db 31, AQUA_JET
	db 33, BUBBLE_BEAM ; Brine → RBY TM move
	db 39, TAKE_DOWN
	db 49, AQUA_TAIL
	db 55, ICE_BEAM
	db 61, AURORA_VEIL
	db 65, HAIL
	db 69, MEGAHORN ; Let's Go move
	db 0 ; no more level-up moves

GrimerPlainLearnset:
GrimerAlolanLearnset:
MukPlainLearnset:
MukAlolanLearnset:
	db 1, MOONBLAST ; Let's Go move
	db 1, STRIKE ; POUND
	db 1, ACID ; Poison Gas → new move
	db 4, HARDEN
	db 7, BITE
	db 12, DISABLE
	db 15, VENOSHOCK ; Acid Spray → tutor move
	db 18, HAZE ; Poison Fang → egg move
	db 21, MINIMIZE
	db 29, KNOCK_OFF
	db 32, CRUNCH
	db 37, SCREECH
	db 40, GUNK_SHOT
	db 46, ACID_ARMOR
	db 52, EXPLOSION ; Belch → TM move
	db 0 ; no more level-up moves

ShellderLearnset:
CloysterLearnset:
	db 1, HYDRO_PUMP
	db 1, SHELL_SMASH
	db 1, FURY_STRIKES ; Spike Cannon → similar move
	db 1, TRI_ATTACK ; RBY TM move
	db 1, HARDEN ; WITHDRAW
	db 1, SUPERSONIC
	db 1, PROTECT
	db 1, AURORA_BEAM
	db 28, ICICLE_SPEAR
	db 32, PIN_MISSILE ; Sw/Sh TM move
	db 50, ICICLE_CRASH
	db 0 ; no more level-up moves

GastlyLearnset:
HaunterLearnset:
GengarLearnset:
	db 1, HYPNOSIS
	db 1, LICK
	db 5, DISABLE ; Spite → egg move
	db 15, NIGHT_SHADE
	db 19, CONFUSE_RAY
	db 22, SUCKER_PUNCH
	db 33, SHADOW_BALL
	db 39, DREAM_EATER
	db 44, DARK_PULSE
	db 55, HEX
	db 0 ; no more level-up moves

OnixLearnset:
SteelixLearnset:
	db 1, STRIKE ; TACKLE
	db 1, HARDEN
	db 1, WRAP ; Bind → similar move
	db 7, ROCK_THROW
	db 10, FURY_CUTTER
	db 13, ROCK_BLAST ; Rock Tomb → egg move
	db 16, BODY_SLAM ; Stealth Rock → TM move
	db 19, SANDSTORM ; Rock Polish → Sandstorm
	db 22, ANCIENTPOWER ; Smack Down → HGSS tutor move
	db 25, DRAGONBREATH
	db 28, HEADBUTT ; Slam → tutor move
	db 31, SCREECH
	db 34, ROCK_SLIDE
	db 37, CRUNCH ; Sand Tomb → Steelix move
	db 40, IRON_TAIL
	db 43, DIG
	db 46, STONE_EDGE
	db 49, DOUBLE_EDGE
	db 52, EARTHQUAKE ; Sandstorm → TM move
	db 0 ; no more level-up moves

DrowzeeLearnset:
HypnoLearnset:
	db 1, STRIKE ; POUND
	db 1, HYPNOSIS
	db 5, DISABLE
	db 9, CONFUSION
	db 13, HEADBUTT
	db 17, LOW_KICK ; Poison Gas → HGSS tutor move
	db 21, CALM_MIND ; Meditate → TM move
	db 25, PSYBEAM
	db 29, TRI_ATTACK ; Wake-Up Slap → RBY TM move
	db 33, BARRIER ; Psych Up → egg move
	db 37, BELLY_DRUM ; Synchronoise → event move
	db 41, ZEN_HEADBUTT
	db 49, PSYCHIC_M
	db 53, NASTY_PLOT
	db 0 ; no more level-up moves

KrabbyLearnset:
KinglerLearnset:
	db 1, AQUA_JET ; Bubble → new move
	db 5, STRIKE ; SCRATCH ; Vice Grip → new move
	db 9, LEER
	db 11, HARDEN
	db 15, BUBBLE_BEAM
	db 19, DIG ; Mud Shot → egg move
	db 21, METAL_CLAW
	db 25, STOMP
	db 32, PROTECT
	db 37, X_SCISSOR ; Guillotine → TM move
	db 44, CLOSE_COMBAT ; Slam → new move
	db 51, CRABHAMMER ; Brine → Crabhammer
	db 56, ANCIENTPOWER ; Crabhammer → HGSS tutor move
	db 63, FLAIL ; Flail → new move
	db 0 ; no more level-up moves

VoltorbLearnset:
ElectrodeLearnset:
	db 1, STRIKE ; TACKLE
	db 1, THUNDERSHOCK ; Let's Go move
	db 11, ROLLOUT
	db 13, SCREECH
	db 16, THUNDER_WAVE ; Charge Beam → TM move
	db 20, SWIFT
	db 22, THUNDERBOLT ; Electro Ball → TM move
	db 26, REFLECT ; Self-Destruct → TM move
	db 29, LIGHT_SCREEN
	db 36, EXPLOSION ; Magnet Rise → Explosion
	db 41, THUNDER ; Discharge → TM move
	db 47, BUG_BUZZ ; Explosion → new move
	db 54, GYRO_BALL
	db 58, MIRROR_COAT
	db 0 ; no more level-up moves

ExeggcuteLearnset:
ExeggutorPlainLearnset:
ExeggutorAlolanLearnset:
	db 1, STRIKE ; TACKLE ; Barrage → new move
	db 1, HYPNOSIS
	db 1, CONFUSION
	db 1, DRAGON_PULSE ; evolution move
	db 27, ZEN_HEADBUTT ; Egg Bomb → tutor move
	db 37, POWER_WHIP ; Wood Hammer → new move
	db 47, OUTRAGE ; Leaf Storm → S/M TM move
	db 0 ; no more level-up moves

CuboneLearnset:
MarowakPlainLearnset:
MarowakAlolanLearnset:
	db 1, SWORDS_DANCE ; evolution move
	db 1, GROWL
	db 3, STRIKE ; TACKLE
	db 7, FAKE_OUT
	db 11, FLAME_CHARGE ; ???
	db 13, LEER
	db 17, HEX
	db 21, BONEMERANG
	db 23, WILL_O_WISP
	db 27, SHADOW_BONE
	db 33, THRASH
	db 37, KNOCK_OFF
	db 43, ACROBATICS
	db 49, BRICK_BREAK
	db 53, FLARE_BLITZ
	db 65, OUTRAGE ; Bone Rush → HGSS tutor move
	db 0 ; no more level-up moves

HitmonleeLearnset:
HitmontopLearnset:
	db 1, DOUBLE_KICK ; evolution move
	db 1, BULK_UP ; Revenge → TM move
	db 5, AGILITY ; Meditate → new move
	db 9, PURSUIT ; Rolling Kick → egg move
	db 13, LOW_KICK ; Jump Kick → HGSS tutor move
	db 17, STOMP ; Brick Break → new move
	db 25, AERIAL_ACE ; Feint → new move
	db 29, HI_JUMP_KICK
	db 33, DOUBLE_TEAM ; Mind Reader → TM move
	db 45, BLAZE_KICK
	db 53, FOCUS_BLAST ; Mega Kick → TM move
	db 57, CLOSE_COMBAT
	db 61, COUNTER ; FLAIL → TM move
	db 0 ; no more level-up moves

TyrogueLearnset:
HitmonchanLearnset:
	db 1, BULK_UP ; Revenge → TM move
	db 6, AGILITY
	db 11, PURSUIT
	db 13, MACH_PUNCH
	db 17, BULLET_PUNCH
	db 21, AERIAL_ACE ; Feint → new move
	db 26, SEISMIC_TOSS ; Vacuum Wave → RBY TM move
	db 36, THUNDERPUNCH
	db 36, ICE_PUNCH
	db 36, FIRE_PUNCH
	db 46, DYNAMICPUNCH ; Mega Punch → TM move
	db 51, PROTECT ; Detect → TM move
	db 56, FOCUS_BLAST ; Focus Punch → TM move
	db 61, COUNTER
	db 66, CLOSE_COMBAT
	db 0 ; no more level-up moves

LickitungLearnset:
LickilickyLearnset:
	db 1, GUST ; event move
	db 1, STRIKE ; TACKLE
	db 1, WATER_GUN ; Bubble → similar move
	db 3, SUPERSONIC
	db 7, BUBBLE_BEAM
	db 11, CONFUSE_RAY
	db 14, WING_ATTACK
	db 16, HEADBUTT
	db 19, WATER_PULSE
	db 23, AQUA_JET ; Wide Guard → new move
	db 27, TAKE_DOWN
	db 32, AGILITY
	db 36, AIR_SLASH
	db 39, ROOST ; Aqua Ring → similar move
	db 46, MIRROR_COAT ; Bounce → Dream World move
	db 49, HYDRO_PUMP
	db 0 ; no more level-up moves

KoffingLearnset:
WeezingPlainLearnset:
WeezingGalarianLearnset:
	db 1, STRIKE ; TACKLE
	db 4, ACID ; Smog → new move
	db 7, SMOKESCREEN
	db 15, WILL_O_WISP ; Clear Smog → TM move
	db 18, VENOSHOCK ; Sludge → tutor move
	db 23, EXPLOSION
	db 26, HAZE
	db 29, GYRO_BALL
	db 34, SLUDGE_BOMB
	db 40, EXPLOSION
	db 57, FIRE_BLAST ; Memento → TM move
	db 99, STRANGESTEAM
	db 0 ; no more level-up moves

RhyhornLearnset:
RhydonLearnset:
RhyperiorLearnset:
	db 1, OUTRAGE ; Hammer Arm → HGSS tutor move ; evolution move
	db 1, STRIKE ; TACKLE
	db 1, SAND_ATTACK
	db 1, LEER ; Tail Whip → similar move
	db 3, HORN_ATTACK
	db 6, FURY_STRIKES ; Fury Attack → similar move
	db 17, STOMP
	db 21, BULLDOZE
	db 25, ANCIENTPOWER ; Chip Away → HGSS tutor move
	db 29, ROCK_BLAST
	db 33, DOUBLE_EDGE ; Drill Run → event move
	db 37, TAKE_DOWN
	db 41, STONE_EDGE
	db 48, EARTHQUAKE
	db 55, CLOSE_COMBAT ; Megahorn → new move
	db 62, MEGAHORN ; Horn Drill → Megahorn
	db 81, SMART_STRIKE
	db 0 ; no more level-up moves

HappinyLearnset:
ChanseyLearnset:
BlisseyLearnset:
	db 1, DEFENSE_CURL
	db 1, STRIKE ; POUND
	db 1, GROWL
	db 5, LEER ; Tail Whip → similar move
	db 16, RECOVER
	db 20, METRONOME ; Bestow → egg move
	db 23, MINIMIZE
	db 27, TAKE_DOWN
	db 31, SING
	db 35, CHARM ; Fling → Happiny move
	db 39, HEAL_BELL ; Heal Pulse → HGSS tutor move
	db 44, BODY_SLAM ; Egg Bomb → TM move
	db 50, LIGHT_SCREEN
	db 57, SUBSTITUTE ; Healing Wish → TM move
	db 65, DOUBLE_EDGE
	db 0 ; no more level-up moves

TangelaLearnset:
TangrowthLearnset:
	db 1, WRAP
	db 4, SLEEP_POWDER
	db 7, VINE_WHIP
	db 10, ABSORB
	db 14, POISONPOWDER
	db 20, GROWTH
	db 23, MEGA_DRAIN
	db 27, KNOCK_OFF
	db 30, STUN_SPORE
	db 33, HEALINGLIGHT
	db 36, GRASS_KNOT
	db 38, ANCIENTPOWER
	db 41, BODY_SLAM
	db 50, POWER_WHIP
	db 0 ; no more level-up moves

KangaskidLearnset:
KangaskhanLearnset:
	db 1, STRIKE ; POUND
	db 1, LEER
	db 1, FLAIL ; Kangaskid
	db 8, FAKE_OUT
	db 16, BITE
	db 24, DRAIN_PUNCH
	db 32, CRUNCH
	db 40, OUTRAGE
	db 48, SUCKER_PUNCH
	db 0 ; no more level-up moves

HorseaLearnset:
SeadraLearnset:
KingdraLearnset:
	db 1, WATER_GUN
	db 1, LEER
	db 3, SMOKESCREEN
	db 9, BUBBLE_BEAM
	db 15, AQUA_JET ; Twister → new move
	db 21, DRAGONBREATH
	db 27, WATERFALL
	db 33, AGILITY
	db 39, DRAGON_PULSE
	db 45, HYDRO_PUMP
	db 51, RAIN_DANCE
	db 0 ; no more level-up moves

GoldeenLearnset:
SeakingLearnset:
	db 1, PECK
	db 1, LEER ; Tail Whip → similar move
	db 5, SUPERSONIC
	db 10, WATER_GUN ; Water Pulse?
	db 15, HORN_ATTACK
	db 20, FURY_STRIKES ; Fury Attack → similar moves
	db 25, FLAIL ; Flail → similar move
	db 30, WATERFALL
	db 35, AGILITY
	db 40, BODY_SLAM ; SMART STRIKE
	db 45, MEGAHORN
	db 0 ; no more level-up moves

StaryuLearnset:
StarmieLearnset:
	db 1, HYDRO_PUMP
	db 1, WATER_GUN
	db 1, RAPID_SPIN
	db 1, RECOVER
	db 1, SWIFT
	db 40, CONFUSE_RAY
	db 60, TRI_ATTACK ; RBY TM move
	db 0 ; no more level-up moves

MimeJrLearnset:
MrMimePlainLearnset:
MrMimeGalarianLearnset:
MrRimeLearnset:
	db 1, STRIKE ; POUND
	db 1, BARRIER
	db 6, CONFUSION
	db 10, FURY_STRIKES
	db 16, MIMIC
	db 20, PROTECT
	db 26, RECYCLE
	db 30, ENCORE
	db 36, PSYBEAM
	db 40, SUCKER_PUNCH
	db 46, LIGHT_SCREEN
	db 46, REFLECT
	db 50, SUBSTITUTE
	db 56, DAZZLINGLEAM
	db 60, PSYCHIC_M
	db 0 ; no more level-up moves

ScytherLearnset:
ScizorLearnset:
KleavorLearnset:
	db 1, QUICK_ATTACK
	db 1, LEER
	db 7, FURY_CUTTER
	db 10, PURSUIT
	db 14, NIGHT_SLASH ; FALSE_SWIPE
	db 17, DOUBLE_TEAM
	db 20, GLARE ; FEINT
	db 24, WING_ATTACK
	db 27, AGILITY ; Razor Wind → new move
	db 30, SLASH
	db 34, BUG_BITE ; RAZOR_WIND
	db 37, AIR_SLASH
	db 40, SWORDS_DANCE
	db 44, X_SCISSOR
	db 0 ; no more level-up moves

SmoochumLearnset:
JynxLearnset:
	db 1, STRIKE ; POUND
	db 1, LICK
	db 1, CONFUSION
	db 8, SCREECH
	db 11, FURY_STRIKES
	db 18, ICE_PUNCH
	db 21, BODY_SLAM
	db 25, LOVELY_KISS
	db 33, PSYCHIC_M
	db 60, BLIZZARD
	db 0 ; no more level-up moves

ElekidLearnset:
ElectabuzzLearnset:
ElectivireLearnset:
; Add Low Kick
	db 1, QUICK_ATTACK
	db 1, LEER
	db 6, THUNDERSHOCK
	db 12, SWIFT
	db 18, THUNDER_WAVE
	db 24, LIGHT_SCREEN
	db 30, THUNDERPUNCH
	db 36, SCREECH ; Discharge → Screech
	db 42, THUNDERBOLT
	db 48, THUNDER
	db 0 ; no more level-up moves

MagbyLearnset:
MagmarLearnset:
MagmortarLearnset:
; Add a Poison type move to counterpart Elekid's Low Kick
	db 1, SMOKESCREEN
	db 1, LEER
	db 6, EMBER
	db 12, FIRE_SPIN
	db 18, LOW_KICK
	db 24, CONFUSE_RAY
	db 30, FIRE_PUNCH
	db 36, SUNNY_DAY ; Lava Plume → Sunny Day
	db 42, FLAMETHROWER
	db 48, FIRE_BLAST
	db 0 ; no more level-up moves

PinsirLearnset:
	db 1, WRAP ; Bind → similar move
	db 1, HARDEN
	db 8, SEISMIC_TOSS
	db 14, FURY_STRIKES ; Vice Grip
	db 20, BUG_BITE
	db 26, BRICK_BREAK
	db 32, X_SCISSOR
	db 38, SWORDS_DANCE
	db 44, THRASH
	db 50, CLOSE_COMBAT ; Superpower
	db 0 ; no more level-up moves

TaurosLearnset:
	db 1, STRIKE ; TACKLE
	db 6, LEER ; Tail Whip → similar move
	db 12, HORN_ATTACK
	db 18, DOUBLE_KICK ; ADDITIONAL
	db 24, BULK_UP ; WORK_UP
	db 30, REST
	db 34, TAKE_DOWN
	db 38, PURSUIT
	db 42, HEADBUTT
	db 46, THRASH
	db 50, DOUBLE_EDGE
	db 54, OUTRAGE
	db 0 ; no more level-up moves

MiltankLearnset:
	db 1, STRIKE ; TACKLE
	db 6, GROWL
	db 12, ROLLOUT
	db 18, STOMP
	db 24, DEFENSE_CURL
	db 30, RECOVER
	db 34, TAKE_DOWN
	db 38, PLAY_ROUGH
	db 42, CHARM
	db 46, BODY_SLAM
	db 50, DOUBLE_EDGE
	db 54, HEAL_BELL
	db 0 ; no more level-up moves

MagikarpLearnset:
	db 1, SPLASH
	db 15, STRIKE ; TACKLE
	db 25, FLAIL ; Flail
	db 0 ; no more level-up moves

GyaradosLearnset:
; Add DRAGON DANCE?
	db 1, BITE ; evolution move
	db 1, FLAIL ; Flail
	db 1, THRASH
	db 1, FIRE_SPIN ; WHIRLPOOL
	db 5, LEER
	db 10, DRAGONBREATH
	db 15, WRAP
	db 20, DRAGON_PULSE ; DRAGON_TAIL
	db 25, WATERFALL
	db 30, CRUNCH
	db 35, RAIN_DANCE
	db 40, AQUA_TAIL
	db 45, OUTRAGE
	db 50, HURRICANE
	db 55, HYDRO_PUMP
	db 60, HYPER_BEAM
	db 0 ; no more level-up moves

LaprasLearnset:
	db 1, SING
	db 1, WATER_GUN
	db 6, GROWL
	db 12, HAZE
	db 18, ICE_SHARD
	db 24, CONFUSE_RAY
	db 30, WATER_PULSE
	db 36, BODY_SLAM
	db 42, ICE_BEAM
	db 48, SURF
	db 54, DRAGON_PULSE
	db 60, BLIZZARD
	db 66, HYDRO_PUMP
	db 72, RAIN_DANCE
	db 0 ; no more level-up moves

EeveeLearnset:
VaporeonLearnset:
JolteonLearnset:
FlareonLearnset:
EspeonLearnset:
UmbreonLearnset:
LeafeonLearnset:
GlaceonLearnset:
SylveonLearnset:
	db 1, STRIKE ; TACKLE
	db 1, GROWL
	db 1, CLEAR_SMOG ; Tail Whip → similar move
	db 13, QUICK_ATTACK
	db 5, SAND_ATTACK
	db 9, DOUBLE_KICK ; Baby-Doll Eyes → Let's Go move
	db 29, CHARM
	db 10, SWIFT
	db 17, BITE
	db 20, HEAL_BELL ; Mimic
	db 25, TAKE_DOWN
	db 33, HELPING_HAND
	db 37, DOUBLE_EDGE
	db 0 ; no more level-up moves

PorygonLearnset:
Porygon2Learnset:
PorygonZLearnset:
	db 1, STRIKE ; TACKLE
	db 5, PSYBEAM
	db 9, AGILITY
	db 14, BARRIER
	db 18, TRI_ATTACK
	db 23, THUNDER_WAVE
	db 27, CONVERSION
	db 32, RECOVER
	db 0 ; no more level-up moves

OmanyteLearnset:
OmastarLearnset:
	db 1, CRUNCH ; evolution move
	db 1, WRAP
	db 1, HARDEN ; DEFENSE_CURL
	db 6, ROLLOUT
	db 12, LEER
	db 18, WATER_GUN
	db 24, BITE
	db 30, ROCK_THROW
	db 36, PROTECT
	db 42, ROCK_BLAST
	db 48, ANCIENTPOWER
	db 54, HYDRO_PUMP
	db 60, SHELL_SMASH
	db 0 ; no more level-up moves

KabutoLearnset:
KabutopsLearnset:
	db 1, SLASH ; evolution move
	db 1, STRIKE ; SCRATCH
	db 1, HARDEN ; DEFENSE_CURL
	db 6, FURY_STRIKES
	db 12, LEER
	db 18, ABSORB
	db 24, AQUA_JET
	db 30, ROCK_THROW
	db 36, PROTECT
	db 42, FURY_CUTTER
	db 48, ANCIENTPOWER
	db 54, LEECH_LIFE
	db 60, SWORDS_DANCE
	db 0 ; no more level-up moves

AerodactylLearnset:
	db 1, WING_ATTACK
	db 1, BITE
	db 7, ROAR
	db 14, SUPERSONIC
	db 21, ANCIENTPOWER
	db 28, AGILITY
	db 35, CRUNCH
	db 42, ROCK_SLIDE
	db 49, FLY
	db 56, IRON_HEAD
	db 63, TAKE_DOWN
	db 70, HYPER_BEAM
	db 0 ; no more level-up moves

MunchlaxLearnset:
SnorlaxLearnset:
	db 1, LICK
	db 1, STRIKE ; TACKLE
	db 1, FLAIL ; FLAIL
	db 6, DEFENSE_CURL
	db 12, YAWN
	db 18, BITE
	db 24, HEADBUTT
	db 30, REST
	db 36, ROLLOUT
	db 42, SCREECH
	db 48, AMNESIA
	db 54, BODY_SLAM
	db 60, CRUNCH
	db 66, OUTRAGE ; HEAVY_SLAM
	db 72, BELLY_DRUM
	db 78, CLOSE_COMBAT ; SUPERPOWER
	db 0 ; no more level-up moves

ArticunoPlainLearnset:
ArticunoGalarianLearnset:
	db 1, GUST
	db 1, ICE_SHARD
	db 7, HAZE
	db 14, LEER
	db 21, ICE_PUNCH ; FREEZE_DRY
	db 28, AGILITY
	db 35, ANCIENTPOWER
	db 42, ICE_BEAM
	db 49, REFLECT
	db 56, ROOST
	db 63, HAIL
	db 70, BLIZZARD
	db 77, HURRICANE
	db 0 ; no more level-up moves

ZapdosPlainLearnset:
ZapdosGalarianLearnset:
	db 1, PECK
	db 1, THUNDERSHOCK
	db 7, THUNDER_WAVE
	db 14, LEER
	db 21, DRILL_PECK
	db 28, AGILITY
	db 35, ANCIENTPOWER
	db 42, THUNDERBOLT
	db 49, LIGHT_SCREEN
	db 56, ROOST
	db 63, RAIN_DANCE
	db 70, THUNDER
	db 77, HURRICANE
	db 0 ; no more level-up moves

MoltresPlainLearnset:
MoltresGalarianLearnset:
	db 1, WING_ATTACK
	db 1, EMBER
	db 7, FIRE_SPIN
	db 14, LEER
	db 21, AIR_SLASH
	db 28, AGILITY
	db 35, ANCIENTPOWER
	db 42, FLAMETHROWER
	db 49, WILL_O_WISP
	db 56, ROOST
	db 63, SUNNY_DAY
	db 70, FIRE_BLAST
	db 77, HURRICANE
	db 0 ; no more level-up moves

DratiniLearnset:
DragonairLearnset:
DragoniteLearnset:
	db 1, WRAP
	db 1, LEER
	db 7, THUNDER_WAVE
	db 14, DRAGONBREATH ; evolution move
	db 21, AGILITY
	db 28, DRAGON_PULSE ; Dragon Tail → GSC TM move
	db 35, AQUA_TAIL
	db 42, BODY_SLAM
	db 49, RAIN_DANCE
	db 56, OUTRAGE
	db 63, HYPER_BEAM
	db 70, ROOST
	db 77, HURRICANE
	db 0 ; no more level-up moves

MewtwoLearnset:
	db 1, DISABLE
	db 1, CONFUSION
	db 1, TELEPORT
	db 8, ANCIENTPOWER
	db 16, PSYCHO_CUT
	db 24, HAZE
	db 32, PSYBEAM
	db 40, SWIFT
	db 48, AMNESIA
	db 56, RECOVER
	db 64, PSYCHIC_M
	db 72, BARRIER
	db 80, AGILITY
	db 88, CALM_MIND
	db 0 ; no more level-up moves

MewLearnset:
	db 1, STRIKE ; POUND
	db 1, CONFUSION
	db 1, MIMIC
	db 11, SWIFT
	db 22, AMNESIA
	db 33, ANCIENTPOWER
	db 44, BARRIER
	db 55, DRAIN_PUNCH
	db 66, METRONOME
	db 77, PSYCHIC_M
	db 88, NASTY_PLOT
	db 99, TRANSFORM
	db 0 ; no more level-up moves

SentretLearnset:
FurretLearnset:
	db 1, STRIKE ; TACKLE
	db 1, AGILITY ; evolution move
	db 4, DEFENSE_CURL
	db 7, QUICK_ATTACK
	db 13, FURY_STRIKES ; Fury Swipes → similar move
	db 17, HELPING_HAND
	db 21, SUPER_FANG ; Follow Me → HGSS tutor move
	db 28, HEADBUTT ; Slam → tutor move
	db 32, REST
	db 36, SUCKER_PUNCH
	db 42, AMNESIA
	db 56, HYPER_VOICE
	db 0 ; no more level-up moves

HoothootLearnset:
NoctowlLearnset:
	db 1, DREAM_EATER
	db 1, STRIKE ; TACKLE
	db 1, GROWL
	db 4, HYPNOSIS
	db 7, PECK
	db 10, CONFUSION
	db 13, NIGHT_SHADE ; Echoed Voice → event move
	db 16, ZEN_HEADBUTT
	db 27, TAKE_DOWN
	db 31, REFLECT
	db 35, AIR_SLASH
	db 43, ROOST
	db 47, HYPER_VOICE ; Moonblast → tutor move
	db 51, MOONBLAST ; Synchronoise → Moonblast
	db 55, DREAM_EATER
	db 0 ; no more level-up moves

SpinarakLearnset:
AriadosLearnset:
	db 1, BUG_BITE ; evolution move
	db 1, POISON_STING
	db 1, STRING_SHOT
	db 5, ABSORB
	db 15, NIGHT_SHADE
	db 23, FURY_STRIKES
	db 28, SUCKER_PUNCH
	db 37, AGILITY
	db 41, PIN_MISSILE
	db 46, PSYCHIC_M
	db 50, POISON_JAB
	db 55, CROSS_POISON
	db 0 ; no more level-up moves

ChinchouLearnset:
LanturnLearnset:
	db 1, WATER_GUN ; Bubble → Water Gun
	db 1, SUPERSONIC
	db 6, THUNDER_WAVE
	db 9, THUNDERSHOCK ; Electro Ball → new move
	db 12, BUBBLE_BEAM ; Water Gun → Bubble Beam
	db 17, CONFUSE_RAY
	db 20, WATER_PULSE ; Bubble Beam → TM move
	db 29, PSYBEAM ; Signal Beam → egg move
	db 33, FLAIL ; Flail → similar move
	db 37, THUNDERBOLT ; Discharge → TM move
	db 43, TAKE_DOWN
	db 47, RECOVER ; Aqua Ring → similar move
	db 51, ICE_BEAM ; Hydro Pump → TM move
	db 54, HYDRO_PUMP ; Ion Deluge → Hydro Pump
	db 58, LIGHT_SCREEN ; Charge → event move
	db 0 ; no more level-up moves

TogepiLearnset:
TogeticLearnset:
TogekissLearnset:
	db 1, DISARM_VOICE ; evolution move
	db 1, HURRICANE ; Sky Attack → new move
	db 1, EXTREMESPEED
	db 1, MOONBLAST
	db 1, AIR_SLASH
	db 1, PLAY_ROUGH ; Sw/Sh move
	db 1, WISH
	db 0 ; no more level-up moves

NatuLearnset:
XatuLearnset:
	db 1, PECK
	db 1, LEER
	db 6, NIGHT_SHADE
	db 12, TELEPORT
	db 18, CONFUSE_RAY
	db 24, WISH
	db 30, PSYCHIC_M
	db 0 ; no more level-up moves

MareepLearnset:
FlaaffyLearnset:
AmpharosLearnset:
	db 1, STRIKE ; TACKLE
	db 1, GROWL
	db 8, THUNDER_WAVE
	db 16, THUNDERSHOCK
	db 24, TAKE_DOWN
	db 32, CONFUSE_RAY
	db 40, THUNDERBOLT ; Discharge → TM move
	db 48, DAZZLINGLEAM
	db 56, LIGHT_SCREEN
	db 64, THUNDER
	db 0 ; no more level-up moves

AzurillLearnset:
MarillLearnset:
AzumarillLearnset:
	db 1, STRIKE ; TACKLE
	db 1, LEER
	db 4, WATER_GUN
	db 8, DEFENSE_CURL
	db 8, ROLLOUT
	db 12, BUBBLE_BEAM
	;db 16, HELPING_HAND
	db 20, AQUA_TAIL
	db 24, PLAY_ROUGH
	db 28, RAIN_DANCE
	db 32, HYDRO_PUMP
	db 36, DOUBLE_EDGE
	db 40, FLAIL ; Superpower
	db 0 ; no more level-up moves

YanmaLearnset:
YanmegaLearnset:
	db 1, STRIKE ; TACKLE
	db 1, GUST
	db 6, QUICK_ATTACK
	db 10, DOUBLE_TEAM
	db 14, AERIAL_ACE
	db 18, PROTECT ; Detect → similar move
	db 22, SUPERSONIC ; Uproar → Supersonic
	db 26, PURSUIT
	db 30, ANCIENTPOWER
	db 34, HYPNOSIS
	db 38, WING_ATTACK
	db 42, SCREECH
	db 46, U_TURN
	db 50, AIR_SLASH
	db 54, BUG_BUZZ
	db 0 ; no more level-up moves

WooperPlainLearnset:
QuagsireLearnset:
	db 1, WATER_GUN
	db 1, LEER ; Tail Whip → similar move
	db 5, RAIN_DANCE
	db 9, BODY_SLAM
	db 15, RECOVER ; YAWN
	db 19, AQUA_TAIL
	db 24, AMNESIA
	db 41, TOXIC
	db 48, EARTHQUAKE
	db 0 ; no more level-up moves

WooperPaldeanLearnset:
ClodsireLearnset:
	db 1, POISON_STING
	db 1, LEER ; Tail Whip → similar move
	db 5, STRIKE ; TACKLE
	db 9, BODY_SLAM
	db 15, RECOVER ; YAWN
	db 19, POISON_JAB
	db 24, AMNESIA
	db 41, TOXIC
	db 48, EARTHQUAKE
	db 0 ; no more level-up moves

MurkrowLearnset:
HonchkrowLearnset:
	db 1, PECK
	db 5, PURSUIT
	db 10, HAZE
	db 15, WING_ATTACK
	db 20, NIGHT_SHADE
	;db 25, TAUNT
	db 30, NIGHT_SLASH
	db 35, AIR_SLASH ; Foul Play → Night Slash
	db 40, NASTY_PLOT
	;db 45, FOUL_PLAY
	db 50, SUCKER_PUNCH
	db 55, DARK_PULSE
	db 0 ; no more level-up moves

MisdreavusLearnset:
MismagiusLearnset:
	db 1, GROWL
	db 1, CONFUSION
	db 10, HYPNOSIS
	db 18, HEX
	db 26, CONFUSE_RAY
	db 34, PSYBEAM
	db 42, SHADOW_BALL
	db 0 ; no more level-up moves

PinecoLearnset:
ForretressLearnset:
	db 1, STRIKE ; TACKLE
	db 1, PROTECT
	db 9, BUG_BITE
	db 12, TAKE_DOWN
	db 17, RAPID_SPIN
	db 28, ROLLOUT
	db 36, EXPLOSION
	db 46, GYRO_BALL
	db 50, DOUBLE_EDGE
	db 0 ; no more level-up moves

GligarLearnset:
GliscorLearnset:
	db 1, POISON_STING
	db 4, SAND_ATTACK
	db 8, HARDEN
	db 12, KNOCK_OFF
	db 16, QUICK_ATTACK
	db 20, FURY_CUTTER
	db 24, ACROBATICS
	db 28, SLASH
	db 32, U_TURN
	db 36, SCREECH
	db 40, X_SCISSOR
	db 44, SWORDS_DANCE
	db 48, EARTHQUAKE
	db 0 ; no more level-up moves

SnubbullLearnset:
GranbullLearnset:
	db 1, STRIKE ; TACKLE
	db 1, LEER ; Tail Whip → similar move
	db 5, CHARM
	db 10, BITE
	db 15, LICK
	db 20, HEADBUTT
	db 25, ROAR
	db 30, FURY_CUTTER ; TAUNT
	db 35, PLAY_ROUGH
	db 40, TAKE_DOWN
	db 45, CRUNCH
	db 0 ; no more level-up moves

ShuckleLearnset:
	db 1, WRAP
	db 5, ROLLOUT
	db 10, ENCORE
	db 15, ROCK_THROW
	db 20, REST
	db 25, SHELL_SMASH
	db 30, ROCK_SLIDE
	db 35, BUG_BITE
	db 40, STONE_EDGE
	db 0 ; no more level-up moves

HeracrossLearnset:
	db 1, STRIKE ; TACKLE
	db 1, LEER
	db 6, FURY_STRIKES
	db 12, AERIAL_ACE ; Fury Attack → Aerial Ace
	db 18, HORN_ATTACK
	db 24, COUNTER
	db 30, SEISMIC_TOSS ; Brick Break → tutor move
	db 36, PIN_MISSILE
	db 42, TAKE_DOWN
	db 48, MEGAHORN
	db 54, CLOSE_COMBAT
	db 0 ; no more level-up moves

SneaselPlainLearnset:
WeavileLearnset:
	db 1, STRIKE ; SCRATCH
	db 1, LEER
	db 6, BITE ; Taunt → egg move
	db 12, QUICK_ATTACK
	db 18, METAL_CLAW
	db 24, ICE_SHARD ; Icy Wind → Ice Shard
	db 30, FURY_STRIKES ; Fury Swipes → similar move
	db 36, AGILITY
	db 42, SCREECH
	db 48, NIGHT_SLASH
	db 0 ; no more level-up moves

SneaselHisuianLearnset:
SneaslerLearnset:
	db 1, STRIKE ; SCRATCH
	db 1, LEER
	db 6, BRICK_BREAK
	db 6, TAUNT
	db 12, QUICK_ATTACK
	db 18, METAL_CLAW
	db 24, POISON_JAB
	db 30, FURY_STRIKES ; BRICK_BREAK
	db 36, AGILITY
	db 42, SCREECH
	db 48, CLOSE_COMBAT
	db 0 ; no more level-up moves

TeddiursaLearnset:
UrsaringLearnset:
UrsalunaLearnset:
	db 1, STRIKE ; SCRATCH
	db 1, LEER
	db 7, LICK
	db 14, FURY_STRIKES ; Fury Swipes → similar move
	db 21, SLASH
	db 28, PLAY_ROUGH
	db 35, CHARM
	db 42, REST
	db 49, THRASH
	db 0 ; no more level-up moves

SlugmaLearnset:
MagcargoLearnset:
	db 1, POISON_GAS
	db 4, EMBER
	db 8, ROCK_THROW
	db 12, HARDEN
	db 16, SMOKESCREEN ; Clear Smog → egg move
	db 20, ANCIENTPOWER
	db 24, ROCK_SLIDE
	db 28, AMNESIA
	db 32, BODY_SLAM
	db 36, RECOVER
	db 40, FLAMETHROWER
	db 0 ; no more level-up moves

SwinubLearnset:
PiloswineLearnset:
MamoswineLearnset:
	db 1, STRIKE ; TACKLE
	db 1, SAND_ATTACK
	db 8, FLAIL
	db 16, ICE_SHARD
	db 24, AMNESIA
	db 32, ICY_WIND
	db 40, TAKE_DOWN
	db 48, EARTHQUAKE
	db 56, BLIZZARD
	db 0 ; no more level-up moves

SkarmoryLearnset:
	db 1, LEER
	db 1, PECK
	db 6, SAND_ATTACK
	db 12, FURY_STRIKES
	db 18, METAL_CLAW
	db 24, SWIFT
	db 30, AGILITY
	db 36, SLASH
	db 42, AIR_SLASH
	db 48, NIGHT_SLASH
	db 0 ; no more level-up moves

HoundourLearnset:
HoundoomLearnset:
	db 1, LEER
	db 1, EMBER
	db 7, SMOKESCREEN ; Smog → new move
	db 14, ROAR
	db 21, BITE
	db 28, PURSUIT ; Taunt
	db 35, WILL_O_WISP ; Foul Play
	db 42, FLAMETHROWER
	db 49, CRUNCH
	db 56, NASTY_PLOT
	db 0 ; no more level-up moves

PhanpyLearnset:
DonphanLearnset:
	db 1, STRIKE ; TACKLE
	db 1, GROWL
	db 1, DEFENSE_CURL
	db 8, ROLLOUT
	db 16, BULLDOZE
	db 24, TAKE_DOWN
	db 32, CHARM
	db 40, EARTHQUAKE
	db 48, DOUBLE_EDGE
	db 0 ; no more level-up moves

LarvitarLearnset:
PupitarLearnset:
TyranitarLearnset:
	db 1, BITE
	db 1, LEER
	db 6, SANDSTORM
	db 12, SCREECH
	db 18, ROCK_SLIDE
	db 24, THRASH
	db 30, DARK_PULSE
	db 36, CRUNCH
	db 42, EARTHQUAKE
	db 48, STONE_EDGE
	db 54, HYPER_BEAM
	db 0 ; no more level-up moves

ChikoritaLearnset:
BayleefLearnset:
MeganiumLearnset:
CyndaquilLearnset:
QuilavaLearnset:
TyphlosionLearnset:
TotodileLearnset:
CroconawLearnset:
FeraligatrLearnset:
SunkernLearnset:
SunfloraLearnset:
UnownLearnset:
QwilfishLearnset:
RaikouLearnset:
EnteiLearnset:
SuicuneLearnset:
LugiaLearnset:
HoOhLearnset:
CelebiLearnset:
DittoLearnset:
NoLearnset::
	db 1, TRANSFORM
	db 0 ; no more level-up moves
